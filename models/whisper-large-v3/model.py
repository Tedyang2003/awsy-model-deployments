...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            audio_input = pb_utils.get_input_tensor_by_name(request, "INPUT")
            audio = audio_input.as_numpy()[0]

            transcribed_text = self.transcriber_pipeline(audio, generate_kwargs={"language":"english"})["text"]
            self.logger.log_info(f"Transcribed Text: {transcribed_text}...")
            inference_response = pb_utils.InferenceResponse(
                output_tensors = [
                    pb_utils.Tensor("OUTPUT", np.array([transcribed_text.encode()]))
                ]
            )
